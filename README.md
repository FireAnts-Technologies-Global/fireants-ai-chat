...
# ANR Studio Ads And Preferences Guide
...
```
if (ANRAdSdk.getInstance().shouldDisplayNativeOnboardingFull1) {
...
- `native_onboarding_full_1`: require
  ...
  `ANRAdSdk.getInstance().shouldDisplayNativeOnboardingFull1 == true`. Remove any legacy
  `organic` flag checks.
- `native_onboarding_full_2`: require
  `AdRemoteConfig.native_onboarding_fullscreen_2_3.isEnable == true` and
  `ANRAdSdk.getInstance().shouldDisplayNativeOnboardingFull2 == true`. Remove `organic`.
- `inter_onboarding`: require `AdRemoteConfig.inter_onboarding.isEnable == true` (or the equivalent
  key defined by your config) and
  `ANRAdSdk.getInstance().shouldDisplayInterOnboarding() == true`. Remove `organic`.
- Widget uninstall builds: do not surface the widget when
  `ANRAdSdk.getInstance().shouldDisplayWidgetUninstall() == false`, regardless of remote
...
## Ads Package Overview (`app/src/main/java/com/anrstudio/template/ads`)
...
1. `GlobalApp` calls `RemoteConfigUtils.init()` and assigns
   `ANRAdSdk.getInstance().prepareLoadingAdsDialogLayout`.
...
- Data model: [`ForceUpdateConfig`](app/src/main/java/com/anrstudio/template/data/model/ForceUpdateConfig.kt).
...
```json
{
  ...
  "storeLink": "https://play.google.com/store/apps/details?id=com.anrstudio.template",
  "minVersionCode": 123,
  "force": true
}
```
