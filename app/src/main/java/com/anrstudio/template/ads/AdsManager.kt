...
import com.anrstudio.ads.util.AppConstant
import com.anrstudio.template.ui.bases.ext.goneView
import com.anrstudio.template.ui.bases.ext.isNetwork
import com.anrstudio.template.ui.component.dialog.DialogLoading
...
object AdsManager {
    ...
    fun loadInterAndShowInterHome(context: Activity, nextAction: () -> Unit) {
        if (!isNetwork(context) || !AdRemoteConfig.inter_home.isEnable || AppPurchase.getInstance()
                .isPurchased(context)
        ) {
            nextAction.invoke()
            return
        ...
        ANRAdSdk.getInstance().getInterstitialAds(
            context,
    ...
    private fun showInterInterval(
        ...
        if (!isNetwork(context) || AppPurchase.getInstance().isPurchased(context)) {
            onNextAction.invoke()
            return
        }


        ANRAdSdk.getInstance().forceShowInterstitial(
            context,
            interstitialAd,
            object : AdCallback() {
    ...
    fun loadNativeFullAll(activity: Activity, onLoaded: (() -> Unit)? = null) {
        if (AdRemoteConfig.native_full_all.isEnable && !AppPurchase.getInstance()
            ...
            ANRAdSdk.getInstance().loadNativeAdResultCallback(
                activity,
    ...
    private fun showNativeFullAllOverlay(activity: Activity, onNextAction: () -> Unit) {
        ...
        try {
            ...
            if (nativeAdFullAll != null) {
                ...
                ANRAdSdk.getInstance().populateNativeAdView(
                    activity,
                    nativeAdFullAll!!,
                    adContainer,
                    shimmer
    ...
    fun loadNativeOnboardingFullAfterPage1(activity: Activity, isFirst: Boolean, layoutRes: Int) {
        ...
        loadNativeInternal(
            ...
            ANRAdSdk.getInstance().shouldDisplayNativeOnboardingFull1,
    ...
    fun loadNativeOnboardingFullAfterPage3(activity: Activity, isFirst: Boolean, layoutRes: Int) {
        ...
        loadNativeInternal(
            ...
            ANRAdSdk.getInstance().shouldDisplayNativeOnboardingFull2,
    ...
    fun loadInterWelcome(context: Context, ignoreLimit: Boolean = false) {
        ...
        if (!config.isEnable
            ...
            || (!ignoreLimit && !ANRAdSdk.getInstance()
    ...
    fun loadBanner(
        ...
        if (adUnitConfig.isEnable) {
            ...
            if (isCollapse) ANRAdSdk.getInstance().loadCollapsibleBanner(
                activity,
                adUnitConfig.id,
    ...
    private fun removeBannerView(activity: Activity, frAds: FrameLayout) {
        try {
            val container =
                frAds.findViewById<FrameLayout>(com.anrstudio.ads.R.id.banner_container)
            if (container != null) {
            ...
            val shimmerFrameLayout = LayoutInflater.from(activity)
                .inflate(com.anrstudio.ads.R.layout.layout_banner_control, null)
            frAds.removeAllViews()
    ...
    fun loadRewardCheckin(activity: Activity, listener: RewardLoadListener) {
        ...
        ANRAdSdk.getInstance().initRewardAds(activity, config.id, object : AdCallback() {
    ...
    fun showRewardCheckin(activity: Activity, onNextAction: () -> Unit, onFailed: () -> Unit) {
        ...
        var isEarned = false
        ANRAdSdk.getInstance().showRewardAds(activity, rewardedAd, object : RewardCallback {
