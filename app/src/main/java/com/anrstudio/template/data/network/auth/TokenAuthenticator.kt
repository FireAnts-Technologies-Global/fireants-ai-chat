...
import com.anrstudio.template.data.network.model.auth.AuthSessionDto
...
class TokenAuthenticator @Inject constructor(
    ...
    private fun refreshTokens(): TokenSession? {
        ...
        val bearer = authPayloadFactory.buildRefreshBearer("POST", AuthRoute.REFRESH, refreshToken)
            ?: return null
    ...
    private fun retryWith(response: Response, accessToken: String): Request? {
        ...
        val path = response.request.url.encodedPath
        val bearer = authPayloadFactory.buildAccessBearer(
    ...
    private fun normalizePath(path: String): String =
        if (path.startsWith("/api/")) path else "/api/v1$path"
