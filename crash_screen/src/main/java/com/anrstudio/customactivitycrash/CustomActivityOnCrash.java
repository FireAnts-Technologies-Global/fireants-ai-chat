package com.anrstudio.customactivitycrash;
...
import com.anrstudio.customactivitycrash.activity.ANRErrorActivity;
import com.anrstudio.customactivitycrash.config.ANRCrashConfig;
...
public final class CustomActivityOnCrash {
    ...
    private static final String EXTRA_CONFIG = "com.anrstudio.customactivitycrash.EXTRA_CONFIG";
    private static final String EXTRA_STACK_TRACE = "com.anrstudio.customactivitycrash.EXTRA_STACK_TRACE";
    private static final String EXTRA_ACTIVITY_LOG = "com.anrstudio.customactivitycrash.EXTRA_ACTIVITY_LOG";
    private static final String EXTRA_CUSTOM_CRASH_DATA = "com.anrstudio.customactivitycrash.EXTRA_CUSTOM_CRASH_DATA";

    //General constants
    private static final String INTENT_ACTION_ERROR_ACTIVITY = "com.anrstudio.customactivitycrash.ERROR";
    private static final String INTENT_ACTION_RESTART_ACTIVITY = "com.anrstudio.customactivitycrash.RESTART";
    private static final String CAOC_HANDLER_PACKAGE_NAME = "com.anrstudio.customactivitycrash";
    ...
    private static ANRCrashConfig config = new ANRCrashConfig();
                    ...
                    Thread.setDefaultUncaughtExceptionHandler((thread, throwable) -> {
                        if (config.isEnabled()) {
                            ...
                            if (hasCrashedInTheLastSeconds(application)) {
                                ...
                                Log.d(TAG, "backgroundMode: " + config.getBackgroundMode() + " (SHOW_CUSTOM=" + ANRCrashConfig.BACKGROUND_MODE_SHOW_CUSTOM + ")");
                                ...
                                } else if (config.getBackgroundMode() == ANRCrashConfig.BACKGROUND_MODE_SHOW_CUSTOM || !isInBackground
                                ...
                                } else if (config.getBackgroundMode() == ANRCrashConfig.BACKGROUND_MODE_CRASH) {
                                    if (oldHandler != null) {
                                        oldHandler.uncaughtException(thread, throwable);
                                        return;
                                    }
     ...
     */
    @Nullable
    public static ANRCrashConfig getConfigFromIntent(@NonNull Intent intent) {
        ANRCrashConfig config = (ANRCrashConfig) intent.getSerializableExtra(CustomActivityOnCrash.EXTRA_CONFIG);
     ...
     */
    public static void restartApplicationWithIntent(@NonNull Activity activity, @NonNull Intent intent, @NonNull ANRCrashConfig config) {
    ...
    public static void restartApplication(@NonNull Activity activity, @NonNull ANRCrashConfig config) {
     ...
     * @param config   The config object as obtained by calling getConfigFromIntent.
     */
    public static void closeApplication(@NonNull Activity activity, @NonNull ANRCrashConfig config) {
        if (config.getEventListener() != null) {
            try {
    ...
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    @NonNull
    public static ANRCrashConfig getConfig() {
        return config;
    }
     ...
     */
    @RestrictTo(RestrictTo.Scope.LIBRARY)
    public static void setConfig(@NonNull ANRCrashConfig config) {
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="com.anrstudio.customactivitycrash.RESTART" />,
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="com.anrstudio.customactivitycrash.RESTART" />,
     ...
     * It will first get activities from the AndroidManifest with intent filter <action android:name="com.anrstudio.customactivitycrash.ERROR" />,
    ...
    private static Class<? extends Activity> guessErrorActivityClass(@NonNull Context context) {
        ...
        if (resolvedActivityClass == null) {
            resolvedActivityClass = ANRErrorActivity.class;
        }
    ...
    /**
     * INTERNAL method used to get the first activity with an intent-filter <action android:name="com.anrstudio.customactivitycrash.ERROR" />,
     * If there is no activity with that intent filter, this returns null.
     *
     * @param context A valid context. Must not be null.
     * @return A valid activity class, or null if no suitable one is found
